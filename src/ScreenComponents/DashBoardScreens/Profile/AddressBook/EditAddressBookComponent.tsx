...
import {useUpdateAddressBookMutation} from '../../../../api/addressBookAPI';
...
const EditAddressBookComponent = ({navigation, route}: Props) => {
  ...
  const [walletIcon, setWalletIcon] = useState(networks?.Wallet_icon);
  const [selectedNetwork, setSelectedNetwork] = useState<any>(networks?.ID);

  const [walletName, setWalletName] = useState(networks?.Wallet_network);
  const [walletAddress, setWalletAddress] = useState(
    walletInfo?.wallet_address,
  );
  const [inputName, setInputName] = useState(walletInfo?.wallet_name);
  const [inputNote, setInputNote] = useState(walletInfo?.wallet_note);
  ...
  const updateAddress = async () => {
    try {
      ...
      const response: any = await updateAddressBook(params).unwrap();
      if (response?.success) {
        showToast({
          type: 'success',
            ...
            <View style={styles.searchContainer}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  Clipboard.setString(walletInfo?.wallet_address);
                }}>
...
export default EditAddressBookComponent;
